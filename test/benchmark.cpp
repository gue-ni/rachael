...
int main(){
    ...
    tic = clock();
    std::optional<Ply> p1 = s1.search(board, WHITE, depth);
    toc = clock();
    std::cout << p1.value();
    printf(" depth=%d, order_moves=%d %f\n", depth, s1.order_moves, dt(tic, toc));

    tic = clock();
    std::optional<Ply> p2 = s2.search(board, WHITE, depth);
    toc = clock();
    std::cout << p2.value();
    printf(" depth=%d, order_moves=%d %f\n", depth, s2.order_moves, dt(tic, toc));

    tic = clock();
    std::optional<Ply> p3 = s3.search(board, WHITE, depth);
    toc = clock();
    std::cout << p3.value();
    printf(" depth=%d, order_moves=%d %f\n", depth, s3.order_moves,  dt(tic, toc));

    tic = clock();
    std::optional<Ply> p4 = s4.search(board, WHITE, depth);
    toc = clock();
    std::cout << p4.value();
